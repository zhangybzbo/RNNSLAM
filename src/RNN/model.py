...
#                pose_cnv3, hidden8 = convLSTM(cnv7b, hidden_state[7], 256, [3, 3], scope='pose3_lstm')
#                pose_cnv2, hidden9 = convLSTM(pose_cnv3, hidden_state[8], 256, [3, 3], scope='pose2_lstm')
#                cnv_flat = tf.reshape(pose_cnv2, [-1, pose_cnv2.get_shape()[1].value*pose_cnv2.get_shape()[2].value*pose_cnv2.get_shape()[3].value])
#                pose_final = tf.layers.dense(inputs=cnv_flat, units=6, activation=None)*0.1
            ...
            depth  = slim.conv2d(icnv1, 1,   [3, 3], stride=1, 
                activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp1')+0.01
...
#                pose_final = tf.layers.dense(inputs=cnv_flat, units=6, activation=None)*0.1
            ...
            icnv1  = slim.conv2d(upcnv1, 16,  [3, 3], stride=1, scope='icnv1')
            depth  = slim.conv2d(icnv1, 1,   [3, 3], stride=1, 
                activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp1')*10.0+0.01 #
