...
class MySystem(object):
    ...
    def refresh_display_content(self, add_pose=False, concat=False, deep=False, keep_first=True):
        if add_pose:
        ...
        if keep_first:
            print(self.keyframes[0].name, '->', self.keyframes[-1].name)
...
def main(args):
    ...
    # make a artificial loop
    image_file_paths.extend(reversed(image_file_paths))
    image_file_paths = image_file_paths[:-bootstrap_steps]
    assert image_file_paths[bootstrap_steps] == image_file_paths[-1]
