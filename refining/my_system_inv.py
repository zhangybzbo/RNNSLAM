...
def main(args):
    ...
    inv_cameras_file_path = os.path.splitext(cameras_file_path)[0] + '_inv.txt'
    invert_depth_file(cameras_file_path, inv_cameras_file_path)
    cameras_file_path = inv_cameras_file_path
    ...
    current_frame_id = len(depth_file_paths)
    count_kf = 0
    start_time = time.time()
    with open(cameras_file_path, 'r') as file:
        for i, depth_file_path in reversed(list(enumerate(depth_file_paths))):
            ...
            keyframe = Keyframe(
                ...
                image=image,
                sparse_points_depths=sparse_points_depths,
                name=os.path.basename(depth_file_path)
            )
            if system.should_add_keyframe(keyframe):
                # insert keyframe
                system.add_keyframe(keyframe, refined_depth_dir=args.refined_depth_dir)
                count_kf += 1
                if args.use_viewer:
            ...
            if i==0 and args.refined_depth_dir is not None:
                write_all_keyframes(system, args.refined_depth_dir)
            if args.use_viewer:
                viewer.update()
...
def invert_depth_file(input_file_path, output_file_path):
    output_file_path = os.path.splitext(input_file_path)[0] + '_inv.txt'
